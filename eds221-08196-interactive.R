rm(list = ls()) # clears global enviro
...
wb_indicators_long <- wb_indicators %>%
  pivot_longer(cols = "2001 [YR2001]":"2020 [YR2020]", #which to squish
               names_to = "year",
...
names(wb_data_tidy) <- c("country", "year",
...
co2_total <- wb_data_tidy %>%
  group_by(country) %>%
  summarise(total_co2_kt = sum(co2_emissions_kt, na.rm = TRUE))
...
ggplot(co2_annual, aes(x = year, y = annual_total_co2_kt)) +
  geom_line()
